...
To store the children, we can either use an array of size \| &Sigma; \| (let's say 128, we only need to support the ASCII encoding in our example). Fast but not memory efficient (a [sparse array/sparsetable](http://goog-sparsehash.sourceforge.net/doc/implementation.html) can reduce the overhead with some slowdown). 

```c++
class node {
    // array of 128 elements, null there is no child for the ASCII letter.
    node* children[128]; 
};
```
...
Another way is to use a simple associative container mapping a character code unit to a child node. A binary search tree or a sorted array if we want to keep the order, or eventually a hash table otherwise. Slower (with potential memory allocations when a new child is added) but more compact even if the alphabet is large.
...
You may have noticed that in the previously exposed trie, some nodes just form a linked list that we could compress. For example the end of the word *rubicundus* is formed by the chain *u* -> *n* -> *d* -> *u* -> *s*. We could compress these chains into one *undus* node instead of five nodes.
...
<figure>
  <img src="{{ site.url }}/images/hat-trie/burst.png"  class="image_caption" />
...
* *Trend*. When a container is created a capital *C* is allocated to the container node. On each successful access, the capital is modified. If the access is a direct hit, the capital is incremented with a bonus *B*, otherwise the capital is decremented by a penalty *M*.
...
```c++
int hash = hash_funct("romulus"); // string to int function
int bucket_index = hash % buckets_array_size; // [0, buckets_array_size)
```

The problem is that two keys can end-up in the same bucket (e.g. `hash_funct("romulus") % 4 == hash_funct("romanus") % 4`). To resolve the problem, all hash tables implements some sort of collisions resolution. 
...
The main problem of the basic chaining implementation is that it's not cache-friendly. In C++, if we use a standard hash table of strings, `std::unordered_map<std::string, int>`, every node access in the list requires two pointers indirection (only one if the [small string optimization](https://stackoverflow.com/questions/10315041/meaning-of-acronym-sso-in-the-context-of-stdstring/10319672#10319672) can be used). One to get the next node of the list and one to compare the key.
...
The goal of the array hash table is to alleviate these inconveniences by storing all the strings of a bucket in an array instead of a linked list. In most cases, we will only provoke one cache-miss (more if the array is large) to resolve the collisions. We also don't need to store superfluous pointers reducing the memory usage of the hash table.

The drawback is that when a string need to be appended at the end of the bucket, a reallocation of the whole array may be needed.
...
The consequence is that on prefix l we may need to do some extra work to filter the elements inside an array hash table node. 

If we are searching for all the words starting with the *ro* prefix, things are easy as when going down the tree leads us to a trie node. We just have to take all the words in the descendants of the trie node.

Things get more complex if we are looking for the words having *roma* as prefix. Going down the tree gets us to a container node while we still have the letters *ma* to check for our prefix. All the suffixes in the container node will not have the *ma* letters as prefix (e.g. *mulus* suffix), we need to do a linear filtering. But as the maximum size of the array hash table is fixed, the complexity of the prefix search remains in *O(k + M)*, where *k* is the size of the prefix and *M* is the number of words matching the prefix, even if we have millions of strings in our HAT-trie. We just have a higher constant factor.

Another consequence of this near-sorted order is that if you want to iterate through all the elements in a sorted order, the iterator has to sort all the elements of the container node each time it hits a new container node. As the maximum size of a container node is still fixed, it shouldn't be too bad, but other structures may be a better fit if the elements need to be sorted.

In the end the HAT-trie offers a good balance between speed and memory usage by sacrificing the order of the elements for a near-sorted order.
